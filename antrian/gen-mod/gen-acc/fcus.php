...
<legend style="background-color:#666666; color:#FFFFFF; padding:5px;">&nbsp;Menu Akses Custom&nbsp;</legend>
<?PHP
echo"<form name='fuser' action='gen-mod/gen-acc/xcus.php?act=setmdl&usrid=$_GET[usrid]' method='post' 
	onsubmit=\"return submitForm(this, 'gen-mod/gen-acc/tcus.php?$hal', 'centercolumn');\">";
?>
<ul>
	<?PHP
		if ($x==0){$x=0;} else{$x=$x;}
		while($arsql_prt=mysqli_fetch_array($xsql_prt)){
			$sql_val_prt="SELECT gac.Access_ID,gac.Data_Active 
				FROM gen_access_custom gac 
				WHERE gac.User_ID='$_GET[usrid]' AND gac.Navigate_ID='$arsql_prt[Navigate_ID]'";
			//echo"$sql_val_prt";
			...
			echo"<li class='parent-li' title='$arsql_prt[Description]'>
				...
				echo"<ul>";
					...
					while($arsql_cld=mysqli_fetch_array($xsql_cld)){
						$sql_val_cld="SELECT gac.Access_ID,gac.Data_Active FROM gen_access_custom gac 
							WHERE gac.User_ID='$_GET[usrid]' AND gac.Navigate_ID='$arsql_cld[Navigate_ID]'";
						//echo"$sql_val_prt";
